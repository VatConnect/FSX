	...
	bool bIsJet, MSLPosOrientStruct *pPosOrient, long lGroundSpeedKts, long lOnGround)
{
	...
	SimConnect.AICreateNonATCAircraft(m_hSimConnect, sFSXType.c_str(), cCallsign, InitPos, p->lOurID);
	...
	for (int i = 0; i < Num; i++)
	{
		...
		{ 
			if (pObj->bOnGround)
			{
				...
				hr += SimConnect.SetDataOnSimObject(m_hSimConnect, POS_AGL_STRUCT_ID, pObj->lFSXObjectID, NULL, 0, sizeof(AGLPosOrientStruct), &P);
			}
			else
				hr += SimConnect.SetDataOnSimObject(m_hSimConnect, POS_MSL_STRUCT_ID, pObj->lFSXObjectID, NULL, 0, sizeof(MSLPosOrientStruct), &P);
		}
	}
	if (hr != S_OK)
		return 0;
...
int CFSXObjects::GetUserState(UserStateStruct *pState)
{
	...
	HRESULT hr = SimConnect.RequestDataOnSimObject(m_hSimConnect, REQ_USERSTATE, USER_STATE_STRUCT_ID, SIMCONNECT_OBJECT_ID_USER,
...
{
	m_bObjAGLSet = false;
	HRESULT hr = SimConnect.RequestDataOnSimObject(m_hSimConnect, REQ_OBJAGL, AGL_STRUCT_ID, FSXObjectID, SIMCONNECT_PERIOD_ONCE);
	if (FAILED(hr))
		return 0;
...
int CFSXObjects::RemoveObject(const String &sCallsign)
{
	...
	//Remove from FSX
	SimConnect.AIRemoveObject(m_hSimConnect, pObj->lFSXObjectID, pObj->lOurID);
...
void CFSXObjects::OnFSXAddedObject(DWORD OurObjectID, DWORD FSXObjectID)
{
	for (int i = 0; i < m_apObjects.GetSize(); i++)
	{
		if (m_apObjects[i]->lOurID == (long)OurObjectID)
		{
			...
			SimConnect.AIReleaseControl(m_hSimConnect, FSXObjectID, OurObjectID);
			
			/*REVISIT -- not needed? Reduces jitter & prevents smoking wheels though, I think
			SimConnect.TransmitClientEvent(m_hSimConnect, FSXObjectID, EVENT_FREEZELAT, 1, 
					SIMCONNECT_GROUP_PRIORITY_HIGHEST, SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);	
			SimConnect.TransmitClientEvent(m_hSimConnect, FSXObjectID, EVENT_FREEZEALT, 1, 
					SIMCONNECT_GROUP_PRIORITY_HIGHEST, SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);	
			SimConnect.TransmitClientEvent(m_hSimConnect, FSXObjectID, EVENT_FREEZEATT, 1, 
					SIMCONNECT_GROUP_PRIORITY_HIGHEST, SIMCONNECT_EVENT_FLAG_GROUPID_IS_PRIORITY);	
			*/
...
{
	HRESULT hr = SimConnect.TransmitClientEvent(m_hSimConnect, FSXObjectID, Event, Param, SIMCONNECT_GROUP_PRIORITY_HIGHEST, 
